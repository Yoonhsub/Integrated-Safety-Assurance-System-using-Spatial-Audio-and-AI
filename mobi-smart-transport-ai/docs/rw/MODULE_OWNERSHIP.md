...
- 타 팀원 소유 모듈 수정이 필요하면 docs/rw/충돌 이슈.md에 기록한다.
...
위 예외는 소유권 변경이 아니라 운영 기록 최신화를 위한 최소 권한이다. 공통 규칙이나 의존성 구조 변경이 필요하면 `docs/rw/충돌 이슈.md`에 기록하고 합의 후 수정한다.
...
문제가 있으면 `docs/rw/충돌 이슈.md`에 기록한다.
