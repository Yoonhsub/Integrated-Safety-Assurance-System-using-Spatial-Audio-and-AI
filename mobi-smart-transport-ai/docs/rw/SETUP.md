...
V2 통합 MVP 고도화 단계에서 새 zip을 받은 팀원은 다음 순서로 읽는다.

```txt
1. docs/rw/README.md
2. docs/rw/V2_SECTION_PLAN.md
3. docs/rw/선행작업의존성 정리.md 또는 docs/rw/선행작업의존성 정리.md
...
```txt
1. 관련 파일 저장
2. docs/rw/공통 진행사항.md의 자기 기록 공간 최신화
3. 검토/패치 섹션이면 자기 디버그 리포트 최신화
4. docs/rw/선행작업의존성 정리.md 관련 섹션이면 상태 최신화
5. 충돌이 있으면 docs/rw/충돌 이슈.md 기록
...
```txt
1. docs/rw/선행작업의존성 정리.md 확인
2. docs/rw/충돌 이슈.md에 기록
...
2. 자기 담당 영역인지 확인
3. 자기 영역이면 패치
4. 타 팀원 영역이면 docs/rw/충돌 이슈.md 기록
```
...
V2 섹션을 시작하기 전 루트에서 먼저 구조 검증을 실행한다.
...
Flutter SDK가 없는 환경에서는 실행하지 않고, 섹션 제출 형식의 `NOT_RUN` 또는 `TEST_STATUS`에 `not run: Flutter SDK unavailable`처럼 기록한다.

V2 mock/live 모드 확인 기준:

```txt
- PUBLIC_DATA_USE_MOCK, FIREBASE_USE_MOCK, FCM_USE_MOCK이 현재 섹션 목표와 일치하는지 확인한다.
- PUBLIC_DATA_BASE_URL은 live public data provider를 사용할 때만 실제 base URL로 지정한다.
- Flutter 앱의 backend baseUrl은 로컬 FastAPI 주소와 배포 주소를 혼동하지 않게 섹션 결과에 기록한다.
- 실행하지 않은 검증은 PASS로 쓰지 않는다.
```

각 팀원이 자기 V2 섹션을 시작하기 전 확인할 항목:

```txt
[ ] docs/rw/V2_SECTION_PLAN.md에서 자기 이름의 Section 번호를 확인했다.
[ ] docs/rw/선행작업의존성 정리.md에서 선행 팀원/섹션을 확인했다.
[ ] docs/rw/공통 진행사항.md의 상태값 규칙을 확인했다.
[ ] 변경 예정 파일이 자기 담당 영역인지 확인했다.
[ ] 사용할 mock/live 모드와 실행할 테스트 명령을 정했다.
```
