...
## V3 버스 탑승 안내 에이전트 (Section 2) — 안내 상태 머신과 세션 저장소

- GuidanceState 12종 enum 확정: IDLE, DESTINATION_SET, ROUTE_RECOMMENDED, ROUTE_SELECTED, NAVIGATING_TO_STOP, ARRIVED_AT_STOP, WAITING_FOR_BUS, BUS_APPROACHING, BOARDING_CONFIRMATION, BOARDED, MISSED_BUS, REPLAN_NEXT_BUS
- ALLOWED_TRANSITIONS 매핑으로 유효하지 않은 상태 전이를 400으로 거부
- GuidanceSession Pydantic 모델에 geofenceArmed, hasArrivedAtStop, targetBusId, nearestBeaconId, lastDecision 등 V3 전용 필드 포함
- in-memory guidance_session_store 구현: get_session, save_session, clear_all
- guidance_state_service: create_or_get_session, get_session_or_404, reset_session, start_guidance, transition
- POST /guidance/session, GET /guidance/state, POST /guidance/state/reset, POST /guidance/start, POST /guidance/transition 엔드포인트 추가
- ARRIVED_AT_STOP 전이 시 geofenceArmed=True, hasArrivedAtStop=True 자동 설정

## V3 버스 탑승 안내 에이전트 (Section 3) — 목적지 기반 노선 추천과 도착 정보 조회

- backend/api/app/fixtures/v3_demo_routes.json 추가: 사창사거리→502, 충북대병원→105, 청주고속버스터미널→747
- route_recommendation_service: recommend_route(destination), get_arrivals(stop_id, route_no)
- 502번 mock 도착 데이터: 6분(BUS_2), 25분(BUS_2_NEXT) — congestion 필드 없음
- POST /bus/route-recommend: destination 기반 노선 추천, 세션 상태 ROUTE_RECOMMENDED로 전이
- GET /bus/arrivals?stopId=&routeNo=: 도착 정보 반환, fallbackSource 포함
- schemas/route_recommendation.py: RouteRecommendRequest, RouteRecommendResponse, ArrivalInfo, BusArrivalsResponse

## V3 버스 탑승 안내 에이전트 (Section 4) — 호출어 기반 음성 에이전트 오케스트레이터

- wake_word_service: detect(utterance, wake_word) → (recognized, normalized_utterance)
- gemini_client: is_available(), extract_intent_and_slots() — API 키 없으면 None 반환
- agent_orchestrator: _rule_intent(utterance) rule-based fallback, process()
- 규칙 우선순위: REPORT_MISSED_BUS("못 탔") → CONFIRM_BOARDED 보다 먼저, GET_BUS_ARRIVAL("언제 와") → SELECT_ARRIVAL 보다 먼저
- 인텐트 9종: FIND_ROUTE, GET_BUS_ARRIVAL, SELECT_ARRIVAL, ASK_CAN_BOARD_CURRENT_BUS, CONFIRM_BOARDED, REPORT_MISSED_BUS, RESET, UNKNOWN, CANCEL
- POST /agent/converse: ConverseRequest(sessionId, utterance, lat, lng) → ConverseResponse(intent, guidanceState, message, ttsMode, cue)
- ASK_CAN_BOARD_CURRENT_BUS: session.lastDecision 기반으로 WRONG_BUS_NEAR→"아니요", TARGET_BUS_NEAR→"네"

## V3 버스 탑승 안내 에이전트 (Section 5) — Flutter V3 안내 화면과 디버그 패널

- apps/passenger_app/lib/src/pages/v3_guidance_page.dart: STT+TTS 통합, mock 버튼 패널, 상태 카드
- apps/passenger_app/lib/src/services/v3_agent_api_client.dart: createSession, converse, mockGeofence, mockBeacons, mockBusEvent, boardingConfirm
- apps/passenger_app/lib/src/models/v3_guidance_models.dart: GuidanceState enum(Dart), GuidanceSession.fromJson(), ConverseResponse.fromJson()
- apps/passenger_app/lib/src/widgets/debug_panel.dart: 15개 필드 실시간 표시
- apps/passenger_app/lib/src/widgets/mock_control_panel.dart: 12개 mock 버튼
- apps/passenger_app/lib/src/widgets/quick_action_panel.dart: 7개 빠른 발화 버튼
- apps/passenger_app/lib/src/pages/home_page.dart AppBar에 V3GuidancePage 진입 버튼 추가
- MOBI_API_BASE_URL 환경변수로 baseUrl 설정 (Android emulator: http://10.0.2.2:8000)

## V3 버스 탑승 안내 에이전트 (Section 6) — 정류장 도착 후 지오펜싱 경고 활성화

- schemas/v3_geofence.py: V3GeofenceStatus enum, MockGeofenceRequest, GeofenceCue, V3GeofenceResponse
- v3_geofence_service: handle_mock_geofence(session_id, mock_status)
- ARRIVED_AT_STOP: 세션 geofenceArmed=True, hasArrivedAtStop=True, 상태→WAITING_FOR_BUS
- LEFT_WAITING_AREA: geofenceArmed=False이면 SAFE(경고 없음), True이면 WARNING+shouldVibrate+ttsMode=SAFETY_LOCAL
- DANGER_ZONE: 항상 DANGER, ttsMode=SAFETY_LOCAL
- RETURNED_TO_STOP: SAFE, geofenceArmed 유지
- POST /mock/geofence 엔드포인트 추가
- geofenceArmed 보호 원칙: 정류장 도착 전 이탈 이벤트는 경고 발생시키지 않음

## V3 버스 탑승 안내 에이전트 (Section 7) — mock 비컨 기반 다중 버스 판별

- schemas/beacon.py: BeaconDecision enum 8종, BeaconEntry, MockBeaconsRequest, BeaconCue, BeaconDecisionResponse
- beacon_decision_service: _LEVEL_ORDER = {"near":0,"mid":1,"far":2}, _nearest(beacons), decide(session_id, beacons)
- RSSI tiebreak: 동일 distanceLevel에서 RSSI 값이 덜 음수인 쪽이 더 가까운 버스로 판정
- 판정 우선순위: NO_BEACON→UNKNOWN(목표 노선 없음)→TARGET_BUS_NOT_FOUND→WRONG_BUS_NEAR→TARGET_BUS_NEAR/MID/FAR
- WRONG_BUS_NEAR: ttsMode=SAFETY_LOCAL, MULTIPLE_BUSES_TARGET_NOT_NEAREST 포함
...
## V3 버스 탑승 안내 에이전트 (Section 8) — 비프음 진동 큐와 선택적 헤드트래킹 표시

- apps/passenger_app/lib/src/models/cue_policy.dart: CueType enum, CuePolicy(near=600ms, mid=1500ms, far=3s, wrongBusNear=2s×3회, danger=500ms)
- apps/passenger_app/lib/src/services/audio_haptic_cue_service.dart: Timer 기반 루프, 중복 방지(_cancel 후 재생), SystemSound+HapticFeedback
- apps/passenger_app/lib/src/models/head_tracking_state.dart: yaw/pitch/roll/relativeYaw/facingDirection, directionFromRelativeYaw(-30~+30=front)
- apps/passenger_app/lib/src/services/head_tracking_service.dart: mock only (실 센서=2학기), calibrate(), updateMock()
- apps/passenger_app/lib/src/widgets/head_tracking_debug_panel.dart: 모든 헤드트래킹 값 + 캘리브레이션 버튼 표시
- 실제 HRTF 공간음향/헤드트래킹 기반 3D 오디오는 2학기 범위로 명시적 제외

## V3 버스 탑승 안내 에이전트 (Section 9) — 탑승 실패 복구와 다음 버스 재안내

- schemas/guidance.py에 BoardingConfirmRequest, BoardingConfirmResponse, BusEventRequest, BusEventResponse 추가
- guidance_state_service.confirm_boarding(session_id, boarded): boarded=True→BOARDED, boarded=False→WAITING_FOR_BUS(previousState=MISSED_BUS, nextArrivalMinutes=25)
- guidance_state_service.handle_bus_event(session_id, event): BUS_PASSED/BUS_ARRIVED→BOARDING_CONFIRMATION, 잘못된 이벤트→400
- REPORT_MISSED_BUS 인텐트: /agent/converse에서 처리, WAITING_FOR_BUS 상태로 복귀 + 다음 버스 시간 안내
- POST /guidance/boarding-confirm 엔드포인트 추가
- POST /mock/bus-event 엔드포인트 추가
- fallbackSource 필드: 다음 버스 정보 출처를 클라이언트에 노출

## V3 버스 탑승 안내 에이전트 (Section 10) — 통합 스모크 테스트와 최종 문서 정리

- scripts/smoke_v3_guidance.py 추가: 14단계 E2E smoke test (httpx 기반)
- 테스트 커버리지: health, session, FIND_ROUTE, arrivals, GET_BUS_ARRIVAL, SELECT_ARRIVAL, ARRIVED_AT_STOP, LEFT_WAITING_AREA, WRONG_BUS_NEAR, ASK_CAN_BOARD, TARGET_BUS_NEAR, BUS_PASSED, REPORT_MISSED_BUS, final state
- V3_API_BASE_URL 환경변수로 대상 서버 지정 (기본값: http://127.0.0.1:8000)
- backend pytest 최종 결과: 128 passed (V3 전체 섹션 누적)
- flutter analyze: No issues found
- docs/rw/V3_SECTION_PLAN.md, V3_API_CONTRACTS.md, V3_DEMO_SCRIPT.md, PATCH_NOTES.md, SETUP.md 최종 갱신
