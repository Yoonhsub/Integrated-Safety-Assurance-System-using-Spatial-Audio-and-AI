...
## V3 section 5 end-to-end smoke / docs packaging patch

- Added `scripts/smoke_v3_guidance.py` with `V3_API_BASE_URL` support and explicit step-level failure messages.
- Added `docs/rw/V3_SECTION_PLAN.md`, `docs/rw/V3_API_CONTRACTS.md`, and `docs/rw/V3_DEMO_SCRIPT.md` so the implemented V3 endpoints, demo flow, fallback policy, and excluded 2학기 scope are documented against actual code.
- Updated setup guidance for backend pytest, FastAPI startup, V3 smoke execution, Flutter `MOBI_API_BASE_URL`, and Android emulator `10.0.2.2`.
- Documented that Gemini is optional and that safety decisions are made by the backend rule engine, not by an LLM.
- Documented public bus API status honestly: no API key is packaged, live provider verification is not claimed, and mock/cache fallback is the verified path.
