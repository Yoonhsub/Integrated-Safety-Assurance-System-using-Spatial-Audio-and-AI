...
타 팀원 담당 영역을 건드리는 브랜치는 생성 전 docs/rw/충돌 이슈.md 기록
```
...
[ ] docs/rw/선행작업의존성 정리.md를 확인했다.
[ ] 현재 섹션이 순서상 맞다.
[ ] 타 팀원 소유 폴더를 수정하지 않는다.
[ ] 선행 작업물이 필요한 경우 구현완료 상태인지 확인했다.
[ ] 충돌 가능성이 있으면 docs/rw/충돌 이슈.md에 먼저 기록했다.
```
...
타 팀원 영역 수정이 필요하면 해당 작업은 멈추고 `docs/rw/충돌 이슈.md`에 기록한다.

---

## 8. 선행작업 의존성 관련 브랜치 규칙

`docs/rw/선행작업의존성 정리.md`에 후행 섹션으로 명시된 작업을 수행할 때는 브랜치 생성 전 선행 섹션 상태를 확인한다.
...
다만 강한 선행작업 의존성이 있는 경우에는 `docs/rw/선행작업의존성 정리.md`를 우선한다.
